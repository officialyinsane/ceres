...
public class Commodity_3 extends AbstractEddnEntity { // TODO: Other properties
    ...
    public static Commodity_3 fromJsonObject(JsonObject obj) throws Exception {
        return Commodity_3.builder()
                ...
                .commodity1List(getAsJsonArray(obj, "commodities", new JsonArray())
                        .asList()
                        .stream()
                        .map(Commodity_1::fromJsonElement)
                        .collect(toList()))
                .economy1List(getAsJsonArray(obj, "economies", new JsonArray())
                        .asList()
                        .stream()
                        .map(Economy_1::fromJsonElement)
                        .collect(toList()))
                .stationName(getAsString(obj, "stationName", null))
                .systemName(getAsString(obj, "systemName", null))
                .prohibitions(getAsJsonArray(obj, "prohibited", new JsonArray())
                        .asList().stream()
